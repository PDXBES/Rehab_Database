...
/****** Object:  StoredProcedure [dbo].[__USP_REHAB_13FillEasyTable_0]    Script Date: 7/16/2019 9:47:37 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  ...
  DECLARE @MaxFailHorizon int = 240--40
...
SET @ENR = @CCICurrent/@CCIBase
 ...
 ,NULL,						geom
  ...
  --Prejudices
  UPDATE  A
  SET     A.prejudiceSpot = B.prejudice
  FROM    REHAB.GIS.nBCR_Data AS A
          INNER JOIN
          REHAB.dbo.[COSTEST_CapitalCostsMobilizationRatesAndTimes] AS B
          ON  A.ID = B.ID
  ...
  UPDATE  A
  SET     A.prejudiceLine = B.prejudice
  FROM    REHAB.GIS.nBCR_Data AS A
          INNER JOIN 
          REHAB.dbo.[COSTEST_CapitalCostsMobilizationRatesAndTimes] AS B
          ON  A.ID = B.ID
  ...
  UPDATE  A
  SET     A.prejudiceDig = B.prejudice
  FROM    REHAB.GIS.nBCR_Data AS A
          INNER JOIN 
          REHAB.dbo.[COSTEST_CapitalCostsMobilizationRatesAndTimes] AS B
          ON  A.ID = B.ID
...
SELECT    Compkey, A.CostToWholePipeOnly AS Cost, LateralCost, 0 AS SpotLateralCost INTO REHAB.GIS.nBCR_View_Costs
  FROM    REHAB.GIS.nBCR_Data AS A
  WHERE (
          ASMRecommendedAction IN ('OC', 'OC Sandwich', '', 'Do Nothing', 'Watch me')
          OR
          ASMRecommendedAction IS NULL
        )
        AND
        cutno = 0
UNION ALL  
SELECT A.Compkey, B.TotalFirstSpotRepairs AS Cost, LateralCost, SpotLateralCost AS SpotLateralCost
  FROM    REHAB.GIS.nBCR_Data AS A
          INNER JOIN
          (  
            SELECT  Z.compkey, 
                    SUM([CapitalNonMobilization]) + MAX([CapitalMobilizationRate])*(SUM(BaseTime) + MAX([MobilizationTime]))/@HoursPerDay AS TotalFirstSpotRepairs,
                    SUM(SegmentLateralCost) AS SpotLateralCost
          ...
          AND cutno = 0
UNION ALL          
SELECT A.Compkey, TotalSpotLineCost AS Cost, LateralCost, 0 AS SpotLateralCost
  FROM    (
