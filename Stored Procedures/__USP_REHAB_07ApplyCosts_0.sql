...
/****** Object:  StoredProcedure [dbo].[__USP_REHAB_07ApplyCosts_0]    Script Date: 7/16/2019 9:39:51 AM ******/
SET ANSI_NULLS ON
GO
...
FROM  #COF_COSTS AS A
      INNER JOIN 
      dbo.[COF-0160a Basement Flooding] AS B
      ON A.GLOBALID = B.GLOBALID
...
FROM  #COF_COSTS AS A
      INNER JOIN 
      dbo.[COF-0160b Sinkhole Freeway] AS B
      ON A.ID = B.ID
...
FROM  #COF_COSTS AS A
      INNER JOIN 
      dbo.[COF-0160c Sinkhole Major Arterial] AS B
      ON A.ID = B.ID
...
FROM  #COF_COSTS AS A
      INNER JOIN 
      dbo.[COF-0160d Sinkhole Residential] AS B
      ON A.ID = B.ID
